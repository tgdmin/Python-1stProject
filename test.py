...
def button(msg, x, y, w, h, inactive_color, active_color, action=None):
    ...
    window.blit(text, (x + (w - text.get_width()) / 2, y + (h - text.get_height()) / 2))


def input_screen_size():
    global width, height
    input_active = True
    input_width = ""
    input_height = ""
    ...
    while input_active:
        ...
        title_text = font_title.render("Snake Game - Set Screen Size", True, blue)
        window.blit(title_text, [width / 2 - title_text.get_width() / 2, 50])

        prompt_width = font_style.render("Width:", True, blue)
        prompt_height = font_style.render("Height:", True, blue)
        window.blit(prompt_width, [width / 4, height / 2 - 50])
        window.blit(prompt_height, [width / 4, height / 2])

        input_width_text = font_style.render(input_width, True, black)
        input_height_text = font_style.render(input_height, True, black)
        window.blit(input_width_text, [width / 2, height / 2 - 50])
        window.blit(input_height_text, [width / 2, height / 2])

        button("Submit", width / 2 - 50, height / 2 + 50, 100, 40, green, red,
               lambda: submit_screen_size(input_width, input_height))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    submit_screen_size(input_width, input_height)
                elif event.key == pygame.K_BACKSPACE:
                    if len(input_width) > 0:
                        input_width = input_width[:-1]
                    elif len(input_height) > 0:
                        input_height = input_height[:-1]
                else:
                    if len(input_width) < 5:  # Limit to reasonable width
                        input_width += event.unicode
                    elif len(input_height) < 5:  # Limit to reasonable height
                        input_height += event.unicode

        pygame.display.update()


def submit_screen_size(input_width, input_height):
    global width, height
    try:
        ...
        message("Invalid screen size! Please enter numbers.", red, y_displace=100)
...
# Start the game with screen size input
input_screen_size()